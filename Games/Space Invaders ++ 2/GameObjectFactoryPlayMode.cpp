...
void GameObjectFactoryPlayMode::buildGameObject(GameObjectBlueprint& bp, vector<GameObject>& gameObjects) {
	...
	for (it; it != end; ++it) {
		if (*it == "Transform") {
        ...
        else if (*it == "Shield Update") {
            gameObject.addComponent(make_shared<ShieldUpdateComponent>());
        }
		else if (*it == "Invader Update") {
			gameObject.addComponent(make_shared<InvaderUpdateComponent>());
		}

        else if (*it == "UFO Update") {
            gameObject.addComponent(make_shared<UfoUpdateComponent>());
        }
			...
			sgp->initializeGraphics(bp.getBitmapName(),Vector2f(bp.getWidth(),
                                                                bp.getHeight()));
		}
	}

	if (bp.getEncompassingRectCollider()) {
		shared_ptr<RectColliderComponent> rcc =make_shared<RectColliderComponent>
